 ...
 */
  constructor ( scan_o )
  {
    ...
    this._hue_a     = null
    this._lum_a     = null
    this._hueCount_a     = null
    this._hueCountMax  = 0
    this._lumCountMax_n  = 0
  ...
  init ()
  {
    try
    {
      ...
      this._capacity      = this._data.length >> 2
...
/**
  ...
  scan ()
  {
    try
    {
      ...
      for ( let at = 0; at < length; at += 4)
      {
        ...
        g = this._data[at+1]
        b = this._data[at+2]
        this._hue_a[RGB_H__i( r, g, b )].push(at)                     // : imageData pointer
        this._lum_a[Math.floor(RGB_L__i( r, g, b ) * 100 )].push(at)  // : idem
      }
                                      //////////////////////
  ...
  getHueCount_a ()
  {
    if ( this._hueCount_a === null)
    {
      this._hueCount_a = new Array( this._hue_n )
      let length = 0
      let count
      for ( let ath=0; ath < this._hue_n; ++ath )
      {
        count = this._hue_a[ath].length
        this._hueCount_a[ath] = count
        if ( count > this._hueCountMax ) this._hueCountMax = count
      }
    }
    return this._hueCount_a
  }

  getHueCountMax ()
  {
    if ( this._hueCount_a === null) this.getHueCount_a()  // : ensure calc has already be done
    return this._hueCountMax
  }
  ...
  HSL_HSLA_a ( atX, atY, ratio )
  {
    ...
    return { h: RGB_H__i( r, g, b ), s: RGB_S__i( r, g, b ), l: RGB_L__i( r, g, b ), a: a }
  }
  ...
  getSaturLumen_a ( hue )
  {
    ...
    for ( let at = 0; at < length; ++at )
    {
      ...
      satur_a[Math.floor(RGB_S__i( r, g, b ) * 100)] += 1
      lumen_a[Math.floor(RGB_L__i( r, g, b ) * 100)] += 1
    }
...
class ColorConsole
{
  constructor ( console_o )
  {
      ...
      this._gridColor      = console_o.gridColor || DOMRootVar__s( '--M3_CONSOLE_COLOR' )
      ...
      DOMRootVar( '--M3_CONSOLE_WIDTH', this._consoleWidth )
      this.init()
  }
  ...
  init ()
  {
    this._paint = SVG( this._selectorId )
      .size( this._consoleWidth, CA_media_o.SLIDE_RANGE + (IMG_PROCESS_o.slider_o.uniheight_n * IMG_PROCESS_o.slider_o.uni_n ) )
  ...
  drawColors ()
  {
    ...
    this._paint
      .rect( this._consoleWidth, CA_media_o.SLIDE_RANGE + (IMG_PROCESS_o.slider_o.uniheight_n * IMG_PROCESS_o.slider_o.uni_n ) )
      .fill( fill )
      return this
  }
 ...
 */
  drawSliders ( )
  {
    const sColor  = DOMRootVar__s( '--M3_CONSOLE_COLOR' )
    const cColor  = DOMRootVar__s( '--M3_CONSOLE_COLOR' )
    let at, atHue, atX
    for ( at = atHue = atX = 0; at < this._slider_n; ++at)
    {
      this._sliders_a[at] = { slider: null, cursor: null }
      this._sliders_a[at].slider = this._paint
          .rect( this._slideWidth, CA_media_o.SLIDE_RANGE )
          .fill( { color: sColor, opacity: 1 } )
          .move( atX, IMG_PROCESS_o.slider_o.top_n )
      this._sliders_a[at].cursor = this._paint
          .line( at * this._slideWidth, IMG_PROCESS_o.slider_o.top_n, (at * this._slideWidth) + this._slideWidth - 2, IMG_PROCESS_o.slider_o.top_n )
  ...
  drawGaps ()  //// HORIZONTALS
  {
   ...
   for ( let at=0, atY = CA_media_o.SLIDE_RANGE - 1; at < IMG_PROCESS_o.slider_o.uni_n; ++at, atY+=IMG_PROCESS_o.slider_o.uniheight_n )
  {
    this._paint
  ...
  handleEvent ( mouse_e )
  {
    ...
    if ( mouse_e.type === 'mousedown' || mouse_e.type === 'mousemove' )
    {
      ...
      if ( atY < CA_media_o.SLIDE_RANGE )  update_o.level = Math.floor( atY ) // : slider level
      else
      { 
        if ( atY > (CA_media_o.SLIDE_RANGE + ( IMG_PROCESS_o.slider_o.uniheight_n * 2 ) ) )  update_o.level = -1    // : hue color monotone
        else if ( atY > (CA_media_o.SLIDE_RANGE + IMG_PROCESS_o.slider_o.uniheight_n ) ) update_o.level = CA_media_o.SLIDE_RANGE   // : maximum opacity 
  ...
  setSlider ( update_o )
  {
    ...
    for (; at < stop; ++at)
    {
      this._sliders_a[at].slider
        .opacity( update_o.level / CA_media_o.SLIDE_RANGE )
      this._sliders_a[at].cursor
        .move( at * this._slideWidth, IMG_PROCESS_o.slider_o.top_n + update_o.level )
...
class ColorBurst
{
  ...
  {
    const sincos = MAIN_o.math.pi2_n * atArc           // MAIN_o.math.pi2_n is full circle (360°)
  ...
  draw ( arcDim )
  {
    ...
    this._colors.forEach( color_o =>
    {
      if ( color_o )
      {
        const position_n = scale.position__i( color_o.frequency )
  ...
  handleEvent ( mouse_e )
  {
    if ( mouse_e.type === 'mousemove' || mouse_e.type === 'click' )
    {
      ...
      let angle = Math.atan2(atY, atX) * MAIN_o.math.radDeg_n
      if ( angle < 0 ) angle += 360
      this.atHue = Math.floor( ( angle + 90 ) % 360 )    // rotate default X axis by -90° to set angle origin at 12:00 not at 3:00
      if ( ( mouse_e.type === 'mousemove' ) && this._onHueTrace ) this._onHueTrace( this.atHue )
      else if ( this._onHueChange ) this._onHueChange( this.atHue )
      return false
    }
  }
...
class OverlaySplitter
{
  ...
  clip ( position )
  {
    const offset = +window.getComputedStyle( this._front_e).width.replace( 'px', '')  // ;LOG( `offset: ${offset}` )                 // : optimum sweep in screen center half
    if ( position === undefined ) position = offset * 0.5
    DOMRootVar( this._clipVar, `${offset - position}px` )
  }
