...
module.exports =
{
  createPost: ( post_o ) =>
  {
    ...
    if ( !post_o.img_src && !post_o.video_src )
    {
      buffer = buffer
      .replace( /`_GALLERY_`/g, '' )
    ...
    {
      buffer = buffer.replace( /_GALLERY_/g, 'gallery' )
      if ( post_o.img_src )
      {
        buffer = buffer
          .replace( /_IMG_SRC_/g,      post_o.img_src )
          .replace( /_IMG_ALT_/g,      post_o.img_alt )
          .replace( /_IMG_TITLE_/g,    post_o.img_title )
          .replace( /_IMG_ABSTRACT_/g, post_o.img_abstract )
      }
      if ( post_o.video_src )
      {
        buffer = buffer
          .replace( /_VIDEO_SRC_/g,      post_o.video_src )
          .replace( /_VIDEO_ALT_/g,      post_o.video_alt )
          .replace( /_VIDEO_TITLE_/g,    post_o.video_title )
          .replace( /_VIDEO_ABSTRACT_/g, post_o.video_abstract )
      }
    }
